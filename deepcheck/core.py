...
TOOL_NAME = "DEEPCHECK"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'DC-C2PA-001', 'severity': 'high', 'weight': 2.5, 'title': 'NO_C2PA_MANIFEST', 'when': {'c2pa_present': False}, 'remediation': 'Treat with caution; provenance unverifiable.'}, {'id': 'DC-DETECT-001', 'severity': 'high', 'weight': 2.5, 'title': 'SYNTHETIC_LIKELY', 'when': {'synthetic_score': '^(0\\.[7-9]|1\\.0)'}, 'remediation': 'Manual review by analyst; flag for moderation.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="synth-media",
                    metadata=item,
                ))
